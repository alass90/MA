...
async def check_agent_count_limit(client, account_id: str) -> Dict[str, Any]:
    try:
        ...
        for agent in agents_result.data or []:
            ...
            if not is_talos_default:
                non_talos_agents.append(agent)
                
        current_count = len(non_talos_agents)
        logger.debug(f"Account {account_id} has {current_count} custom agents (excluding Talos defaults)")

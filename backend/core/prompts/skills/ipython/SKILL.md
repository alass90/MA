---
name: ipython
description: Interactive Python code execution environments for complex computations, data analysis, and chart generation.
---
# Tool Specification: tlstools-ipython
...
- **File System**: 
  - `/workspace/` (read-write user files mapping to E2B)
  - `/dev/shm` (shared memory for inter-process)

### Resource Constraints
- **Memory**: 4GB RAM (CPU-only)
- **Storage**: Ephemeral Sandbox Storage
- **Timeout**: 30s execution timeout per call

## Special Capabilities

### Image Processing
- **Matplotlib**: Automatic figure display (no plt.show() or plt.savefig() needed)
...
### Restrictions
- **Network Blocked**: urllib, requests, wget will fail to connect external APIs

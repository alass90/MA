...
@router.post("/talos-agents/install-user/{account_id}")
async def admin_install_talos_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install Talos agent for a specific user."""
    logger.debug(f"Admin installing Talos agent for user: {account_id}")
    
    service = TalosDefaultAgentService()
    agent_id = await service.install_talos_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Talos agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Talos agent for user {account_id}"
        )

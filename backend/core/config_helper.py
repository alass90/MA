...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for Talos vs custom agents."""
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_talos_default={is_talos_default}")
    ...
    # Handle Talos agents with special logic
    if is_talos_default:
        return _extract_talos_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_talos_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for Talos agents - always use central config with user customizations."""
    from core.talos_config import TALOS_CONFIG
    
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using Talos central config for agent {agent_id}")
    
    # Start with central Talos config
    config = {
...
def build_unified_config(
    ...
    talos_metadata: Optional[Dict[str, Any]] = None,
    ...
    if talos_metadata:
        config['talos_metadata'] = talos_metadata
